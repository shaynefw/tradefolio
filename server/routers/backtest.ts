...
const tradeInputShape = {
  ...
  resultPoints: z.number().nullable().optional(),
  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    const rows = await db
      .select({
        ...
        tpMode: schema.backtestDatasets.tpMode,
        slMode: schema.backtestDatasets.slMode,
  ...
  create: protectedProcedure
    .input(
      z.object({
        ...
        tpMode: z.enum(["fixed", "fluid"]).optional(),
        slMode: z.enum(["fixed", "fluid"]).optional(),
      ...
      return await db.transaction(async (tx) => {
        const [ds] = await tx
          ...
          .values({
            ...
            tpMode: input.tpMode ?? "fixed",
            slMode: input.slMode ?? "fixed",
